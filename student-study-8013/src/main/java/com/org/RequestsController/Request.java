...
public class Request {
    ...
    private static final String BASE_URL = "http://STUDENT-STUDY-8013/app/";

    @RequestMapping("student/shClass")
    public ResponseEntity<ClassResponse> get(HttpServletRequest request){
        ...
        ResponseEntity<ClassResponse> classResponse = restTemplate.postForEntity(BASE_URL+"student/shClass",formEntity,ClassResponse.class);
        for (int i = 0; i < classResponse.getBody().getData().size(); i++) {
            int classId = classResponse.getBody().getData().get(i).getClassId();
            String classEntity= restTemplate.getForObject(BASE_URL+"class/shClass/"+classId,String.class);
    ...
    @RequestMapping("course/type/shTypeByType")
    public ResponseEntity<CourseTypeResponse> shTypeByType(HttpServletRequest request, @RequestBody String body){
        ...
        ResponseEntity<CourseTypeResponse> courseTypeResponseResponseEntity = restTemplate.postForEntity(BASE_URL+"course/type/shTypeByType",formEntity,CourseTypeResponse.class);
        Integer parentId = (Integer) courseTypeResponseResponseEntity.getBody().getData().getParentId();
        if(parentId!=null){
            ResponseEntity<CourseTypeResponse> a = restTemplate.postForEntity(BASE_URL+"course/type/shTypeById/"+parentId,formEntity,CourseTypeResponse.class);
            courseTypeResponseResponseEntity.getBody().getData().setParent(a.getBody().getData());
        }
        return courseTypeResponseResponseEntity;
    }

    @RequestMapping("course/type/shTypeById/{id}")
    public ResponseEntity<CourseTypeResponse> shTypeById(HttpServletRequest request, @PathVariable Long id){
        ...
        ResponseEntity<CourseTypeResponse> courseTypeResponseResponseEntity = restTemplate.postForEntity(BASE_URL+"course/type/shTypeById/"+id,formEntity,CourseTypeResponse.class);
        Integer parentId = (Integer) courseTypeResponseResponseEntity.getBody().getData().getParentId();
        if(parentId!=null){
            ResponseEntity<CourseTypeResponse> a = restTemplate.postForEntity(BASE_URL+"course/type/shTypeById/"+parentId,formEntity,CourseTypeResponse.class);
